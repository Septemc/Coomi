use crate::AdapterError;
use crate::ResponsesApiRequest;
use crate::WireProtocol;
use codex_protocol::models::ContentItem;
use codex_protocol::models::ResponseItem;
use codex_protocol::protocol::TokenUsage;
use codex_tools::JsonSchema;
use serde_json::Value;
use std::collections::HashMap;
use std::collections::HashSet;

pub(crate) fn request_json(request: &ResponsesApiRequest) -> Result<Value, AdapterError> {
    serde_json::to_value(request).map_err(AdapterError::from)
}

pub(crate) fn canonical_tools(
    request: &ResponsesApiRequest,
    protocol: WireProtocol,
) -> Result<Vec<Value>, AdapterError> {
    let request = request_json(request)?;
    let tools = request
        .get("tools")
        .and_then(Value::as_array)
        .cloned()
        .unwrap_or_default();
    reduce_tools_for_protocol(tools, protocol)
}

#[derive(Clone, Debug, PartialEq)]
pub(crate) enum CanonicalWireTool {
    Function {
        namespace: Option<String>,
        name: String,
    },
    ToolSearch {
        parameters: Box<JsonSchema>,
    },
    Freeform {
        name: String,
    },
}

pub(crate) fn canonical_tool_name_map(
    request: &ResponsesApiRequest,
    protocol: WireProtocol,
) -> Result<HashMap<String, CanonicalWireTool>, AdapterError> {
    let request = request_json(request)?;
    let tools = request
        .get("tools")
        .and_then(Value::as_array)
        .cloned()
        .unwrap_or_default();
    let mut mapping = HashMap::new();
    let mut wire_names = HashSet::new();

    for tool in tools {
        match tool.get("type").and_then(Value::as_str) {
            Some("function") => {
                let name = required_tool_string(&tool, "name")?;
                if !wire_names.insert(name.to_string()) {
                    return Err(AdapterError::Config(format!(
                        "duplicate canonical tool name `{name}`"
                    )));
                }
                mapping.insert(
                    name.to_string(),
                    CanonicalWireTool::Function {
                        namespace: None,
                        name: name.to_string(),
                    },
                );
            }
            Some("namespace") => {
                let namespace = required_tool_string(&tool, "name")?;
                let children = tool.get("tools").and_then(Value::as_array).ok_or_else(|| {
                    AdapterError::Config(format!(
                        "tool namespace `{namespace}` is missing a tools array"
                    ))
                })?;
                for child in children {
                    let name = required_tool_string(child, "name")?;
                    let wire_name = wire_tool_name(Some(namespace), name);
                    if !wire_names.insert(wire_name.clone()) {
                        return Err(AdapterError::Config(format!(
                            "tool namespace flattening produced duplicate name `{wire_name}`"
                        )));
                    }
                    mapping.insert(
                        wire_name,
                        CanonicalWireTool::Function {
                            namespace: Some(namespace.to_string()),
                            name: name.to_string(),
                        },
                    );
                }
            }
            Some("tool_search") if protocol != WireProtocol::OpenAiResponses => {
                let parameters = tool
                    .get("parameters")
                    .cloned()
                    .map(serde_json::from_value)
                    .transpose()?
                    .unwrap_or_default();
                insert_special_tool(
                    &mut mapping,
                    &mut wire_names,
                    "tool_search",
                    CanonicalWireTool::ToolSearch {
                        parameters: Box::new(parameters),
                    },
                )?;
            }
            Some("custom") if protocol != WireProtocol::OpenAiResponses => {
                let name = required_tool_string(&tool, "name")?;
                insert_special_tool(
                    &mut mapping,
                    &mut wire_names,
                    name,
                    CanonicalWireTool::Freeform {
                        name: name.to_string(),
                    },
                )?;
            }
            Some("web_search") | None => {}
            Some(_) => {}
        }
    }
    Ok(mapping)
}

fn insert_special_tool(
    mapping: &mut HashMap<String, CanonicalWireTool>,
    wire_names: &mut HashSet<String>,
    wire_name: &str,
    identity: CanonicalWireTool,
) -> Result<(), AdapterError> {
    if !wire_names.insert(wire_name.to_string()) {
        return Err(AdapterError::Config(format!(
            "canonical tool reduction produced duplicate name `{wire_name}`"
        )));
    }
    mapping.insert(wire_name.to_string(), identity);
    Ok(())
}

pub(crate) fn wire_tool_name(namespace: Option<&str>, name: &str) -> String {
    namespace.map_or_else(
        || name.to_string(),
        |namespace| format!("{namespace}__{name}"),
    )
}

fn reduce_tools_for_protocol(
    tools: Vec<Value>,
    protocol: WireProtocol,
) -> Result<Vec<Value>, AdapterError> {
    if protocol == WireProtocol::OpenAiResponses {
        return Ok(tools);
    }

    let mut expanded = Vec::new();
    for tool in tools {
        match tool.get("type").and_then(Value::as_str) {
            Some("function") => expanded.push(tool),
            Some("namespace") => {
                let namespace = required_tool_string(&tool, "name")?;
                let namespace_description = tool
                    .get("description")
                    .and_then(Value::as_str)
                    .unwrap_or_default();
                let children = tool.get("tools").and_then(Value::as_array).ok_or_else(|| {
                    AdapterError::Config(format!(
                        "tool namespace `{namespace}` is missing a tools array"
                    ))
                })?;
                for child in children {
                    if child.get("type").and_then(Value::as_str) != Some("function") {
                        return Err(AdapterError::Config(format!(
                            "tool namespace `{namespace}` contains a non-function tool"
                        )));
                    }
                    let child_name = required_tool_string(child, "name")?;
                    let mut function = child.as_object().cloned().ok_or_else(|| {
                        AdapterError::Config(format!(
                            "tool namespace `{namespace}` contains a non-object tool"
                        ))
                    })?;
                    function.insert(
                        "name".to_string(),
                        Value::String(wire_tool_name(Some(namespace), child_name)),
                    );
                    if !namespace_description.is_empty() {
                        let child_description = child
                            .get("description")
                            .and_then(Value::as_str)
                            .unwrap_or_default();
                        let description = if child_description.is_empty() {
                            namespace_description.to_string()
                        } else {
                            format!("{namespace_description}\n\n{child_description}")
                        };
                        function.insert("description".to_string(), Value::String(description));
                    }
                    expanded.push(Value::Object(function));
                }
            }
            Some("tool_search") => expanded.push(reduce_tool_search(&tool)?),
            Some("custom") => expanded.push(reduce_freeform_tool(&tool)?),
            // Hosted Responses tools have no client runtime behind a Chat-style
            // function call. Omitting them is the only safe deterministic reduction.
            Some("web_search") => {}
            Some(tool_type) => {
                return Err(AdapterError::UnsupportedTool {
                    protocol,
                    tool_type: tool_type.to_string(),
                });
            }
            None => {
                return Err(AdapterError::Config(
                    "canonical tool is missing `type`".to_string(),
                ));
            }
        }
    }
    Ok(expanded)
}

fn reduce_tool_search(tool: &Value) -> Result<Value, AdapterError> {
    Ok(serde_json::json!({
        "type": "function",
        "name": "tool_search",
        "description": required_tool_string(tool, "description")?,
        "parameters": tool.get("parameters").cloned().unwrap_or_else(|| serde_json::json!({"type": "object"})),
        "strict": false,
    }))
}

fn reduce_freeform_tool(tool: &Value) -> Result<Value, AdapterError> {
    let name = required_tool_string(tool, "name")?;
    let description = tool
        .get("description")
        .and_then(Value::as_str)
        .unwrap_or_default();
    let format = tool.get("format");
    let syntax = format
        .and_then(|value| value.get("syntax"))
        .and_then(Value::as_str)
        .unwrap_or("text");
    let definition = format
        .and_then(|value| value.get("definition"))
        .and_then(Value::as_str)
        .unwrap_or_default();
    let wire_description = if definition.is_empty() {
        format!("{description}\n\nPass the complete freeform input verbatim in `input`.")
    } else {
        format!(
            "{description}\n\nPass the complete freeform input verbatim in `input`. The input follows this {syntax} definition:\n{definition}"
        )
    };
    Ok(serde_json::json!({
        "type": "function",
        "name": name,
        "description": wire_description,
        "parameters": {
            "type": "object",
            "properties": {
                "input": {"type": "string", "description": "Complete freeform tool input"}
            },
            "required": ["input"],
            "additionalProperties": false
        },
        "strict": false,
    }))
}

fn required_tool_string<'a>(tool: &'a Value, field: &str) -> Result<&'a str, AdapterError> {
    tool.get(field)
        .and_then(Value::as_str)
        .ok_or_else(|| AdapterError::Config(format!("canonical tool is missing `{field}`")))
}

pub(crate) struct FunctionToolParts<'a> {
    pub name: &'a str,
    pub description: Option<&'a str>,
    pub parameters: Value,
}

pub(crate) fn function_tool_parts(
    protocol: WireProtocol,
    tool: &Value,
) -> Result<FunctionToolParts<'_>, AdapterError> {
    let tool_type = tool
        .get("type")
        .and_then(Value::as_str)
        .unwrap_or("unknown");
    if tool_type != "function" {
        return Err(AdapterError::UnsupportedTool {
            protocol,
            tool_type: tool_type.to_string(),
        });
    }
    let name =
        tool.get("name")
            .and_then(Value::as_str)
            .ok_or_else(|| AdapterError::InvalidPayload {
                protocol,
                message: "function tool is missing `name`".to_string(),
            })?;
    let description = tool.get("description").and_then(Value::as_str);
    let parameters = tool
        .get("parameters")
        .cloned()
        .unwrap_or_else(|| serde_json::json!({"type": "object"}));
    Ok(FunctionToolParts {
        name,
        description,
        parameters,
    })
}

pub(crate) fn response_item_kind(item: &ResponseItem) -> &'static str {
    match item {
        ResponseItem::AdditionalTools { .. } => "additional_tools",
        ResponseItem::Message { .. } => "message",
        ResponseItem::AgentMessage { .. } => "agent_message",
        ResponseItem::Reasoning { .. } => "reasoning",
        ResponseItem::LocalShellCall { .. } => "local_shell_call",
        ResponseItem::FunctionCall { .. } => "function_call",
        ResponseItem::ToolSearchCall { .. } => "tool_search_call",
        ResponseItem::FunctionCallOutput { .. } => "function_call_output",
        ResponseItem::CustomToolCall { .. } => "custom_tool_call",
        ResponseItem::CustomToolCallOutput { .. } => "custom_tool_call_output",
        ResponseItem::ToolSearchOutput { .. } => "tool_search_output",
        ResponseItem::WebSearchCall { .. } => "web_search_call",
        ResponseItem::ImageGenerationCall { .. } => "image_generation_call",
        ResponseItem::Compaction { .. } => "compaction",
        ResponseItem::CompactionTrigger { .. } => "compaction_trigger",
        ResponseItem::ContextCompaction { .. } => "context_compaction",
        ResponseItem::Other => "other",
    }
}

pub(crate) fn text_content(content: &[ContentItem]) -> String {
    content
        .iter()
        .filter_map(|item| match item {
            ContentItem::InputText { text } | ContentItem::OutputText { text } => {
                Some(text.as_str())
            }
            ContentItem::InputImage { .. } | ContentItem::InputAudio { .. } => None,
        })
        .collect::<Vec<_>>()
        .join("\n")
}

pub(crate) fn openai_usage(value: Option<&Value>) -> Option<TokenUsage> {
    let usage = value?.as_object()?;
    let input_tokens = integer(
        usage
            .get("input_tokens")
            .or_else(|| usage.get("prompt_tokens")),
    );
    let output_tokens = integer(
        usage
            .get("output_tokens")
            .or_else(|| usage.get("completion_tokens")),
    );
    let cached_input_tokens = usage
        .get("input_tokens_details")
        .or_else(|| usage.get("prompt_tokens_details"))
        .and_then(|details| details.get("cached_tokens"))
        .map_or(0, |value| integer(Some(value)));
    let reasoning_output_tokens = usage
        .get("output_tokens_details")
        .or_else(|| usage.get("completion_tokens_details"))
        .and_then(|details| details.get("reasoning_tokens"))
        .map_or(0, |value| integer(Some(value)));
    let total_tokens = integer(usage.get("total_tokens")).max(input_tokens + output_tokens);
    Some(TokenUsage {
        input_tokens,
        cached_input_tokens,
        cache_write_input_tokens: 0,
        output_tokens,
        reasoning_output_tokens,
        total_tokens,
    })
}

pub(crate) fn integer(value: Option<&Value>) -> i64 {
    value.and_then(Value::as_i64).unwrap_or_default()
}
