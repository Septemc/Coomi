...
class PlanPanel(Widget):
    ...
    def render(self) -> Table:
        ...
        table.add_row(Text.from_markup(
            f"[bold]Q{self._active_q + 1}: {q['question']}[/bold]"
        ))
        if q.get("recommendation"):
            table.add_row(Text.from_markup(
                f"  [dim]推荐: {q['recommendation']}[/dim]"
            ))
        ...
        for i, opt in enumerate(options):
            ...
            if opt.get("is_other"):
                ...
                if is_sel:
                    display = other_text if other_text else "[dim]输入自定义内容...[/dim]"
                    table.add_row(Text.from_markup(
                        f"  {marker} Other: {display}"
                    ))
                else:
                    display = other_text if other_text else "..."
                    table.add_row(Text.from_markup(
                        f"  {marker} Other: [dim]{display}[/dim]"
                    ))
            else:
                label = f"[bold]{opt['label']}[/bold]" if is_sel else opt["label"]
